...
$fetch_expiring_profiles="select profile_id,first_name,last_name,expiry_date from tbl_basic_profile where CURDATE()=expiry_date";
...
while($fetch_expiring_row = mysqli_fetch_array($fetch_expiring_rs)) 
        ...
        sendExpiryN($profile_id,$name,$expiryDate); 
		}

function  sendExpiryN($profile_id,$name,$expiryDate)
{
//$log = new MyLogPHP('/home/incredib/public_html/marathalagnyog.com/cronjobs/cronlogs/logs.log');
...
while($fetch_contact_row = mysqli_fetch_array($fetch_contact_info_rs)) 
       ...
       // $log->info("Sending Expiry Notice to ::".$profile_id." Name :: ".$name." On ".$mobileNo." Profile Expiry Date is :: ".$expiryDate);
	    ...
	    sendExpiryNoticeSMS($mobileNo,$expiryDate);
        mysqli_query($conn,"UPDATE tbl_basic_profile SET status='E' where profile_id='$profile_id'")or die(mysqli_error($conn));
		}

}

function sendExpiryNoticeSMS($mobno,$expiryDate)
{
//	$log = new MyLogPHP('/home/incredib/public_html/marathalagnyog.com/cronjobs/cronlogs/logs.log');
    $username="maratha";
	...
	$mobile=$mobno;
	$message="Your account is expired on ".$expiryDate." Pay Rs.500 to continue our service.Regards MarathaLagnYog Team Web:www.marathalagnyog.com ";
