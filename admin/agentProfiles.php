    ...
    function savePaymentRecvd()
		...
		b=checkInput(document.paymentrecvd);
   		if (!b) 
			...
			{
			document.paymentrecvd.action="agentProfiles.php?msg=savePaymentRecvd";
...
<div class="ch-container">
    <div class="row">
		...
		<?php
                  ...
                  $rs=mysqli_query($conn,"select first_name ,last_name from tbl_agent_profile where agent_id='$agentId'") or die(mysqli_error($conn));
                  while($rows=mysqli_fetch_array($rs))
                       {
                       $agentName=$rows[0]."&nbsp;".$rows[1];
                       }
                ?>  
		...
		<div class="box-header well" data-original-title="">
			<h2><i class="glyphicon glyphicon-user"></i><?php echo $agentName."(". $_GET['agentId']. ")"; ?> 's Profiles</h2>
			<div class="box-icon">
		...
		<div class="box-content">
		   ...
		   <thead>
			...
			<tbody>
				...
				contactinfo.district,contactinfo.city,basicprofile.status,profiles.status,payments.status from tbl_basic_profile basicprofile,tbl_profile_photo photo,
				tbl_physical_attr phyattr,tbl_contact_info contactinfo,tbl_agent_profiles profiles, tbl_agent_payments payments where basicprofile.profile_id=photo.profile_id 
				and basicprofile.profile_id=phyattr.profile_id  and basicprofile.profile_id=contactinfo.profile_id and basicprofile.profile_id=profiles.profile_id 
				and payments.profile_id=basicprofile.profile_id
				and profiles.agent_id = payments.agent_id 
				and profiles.agent_id='$agentId' order by basicprofile.profile_id desc ";	
				
				$basic_profile_rs= mysqli_query($conn,$fetch_basic_profile) or die("Error while displaying basic profile data:"+mysqli_error($conn));
				...
				while($basic_profile_row = mysqli_fetch_array($basic_profile_rs)) 
					{
					  ...
					  if($basic_profile_row[10]=="Received")
					     echo "<td><a href='agentProfiles.php?agentid=$agentId&profileId=$basic_profile_row[1]&msg=paymentrcvd'><span class='label-warning label label-default'>Mark Payment Received</span></td>";  
					  else 
						  echo "<td><a href='#'><span class='label-success label label-default'>Payment Received</span></td>";  
...
$mess=$mess."<b>Thanks & Regards <BR>Vivahyog Team</b><br>";
...
function sendSMS($name,$mobno,$profileId,$pwd)
{
	...
	$mobile=$mobno;
	$message="Hi ".$name." Your Account Created Successfully.Profile Id:".$profileId." & Password: ".$pwd." Regards Vivahyog Team Web:www.vivahyog.in ";
...
function saveBasicProfile()
{
	...
	$basic_profile_insert = "INSERT INTO tbl_basic_profile(first_name,last_name,gender,
														 ...
														 password,create_datetime,ip_address,status,album_status,age)".
							"VALUES('$firstName','$lastName','$gender',
									'$dateOfBirth','$maritalStatus','$religion','$caste',
									'$subCaste','$password','$create_datetime','$ipAddress','A','Private','$age')";
...
function saveContactInfo($profile_id)
{
	...
	$contact_info_insert="INSERT INTO tbl_contact_info(mobile_no,alternate_mobile_no,email_Id,address,
										   country,state,district,city,profile_id)".
						  "VALUES('$mobileNo','$alternateMobileNo','$emailId','$address',
								  '$country','$state','$district','$city','$profile_id')";	
	mysql_query($contact_info_insert) or die(mysql_error());	
}						  
...
function saveProfilePhoto($profile_id)
{
	...
	if($fileSize <= 0)
		...
		mysql_query($profile_photo_insert) or die(mysql_error());
		}
	else
		{
...
mysql_query($socio_religious_attr_insert) or die(mysql_error()); 
}
...
mysqli_query($conn,$familydetails_insert) or die(mysql_error($conn)); 
}

/* Code to Insert Partner Preference*/
function savePartnerPreference($profile_id)
{
$partner_pref_insert="INSERT INTO tbl_partner_preference(profile_id) values('$profile_id')";
...
function insertAgentDetails($profile_id)
     {
		...
		$fetch_existing_records_rs=mysqli_query($conn,$fetch_existing_records) or die(mysql_error());
		...
		if($numrows > 0)
			...
			mysqli_query($conn,$agent_insert_profiles) or die(mysqli_error());
	...
	?>
    	<div id="boxes">
				<div style="top:150px; left: 551.5px; display: none;" id="dialog" class="window">
					...
					<?php
					echo "<a href='agentMyProfiles.php'><img src='images/cross.png' width='16' style='border:none; cursor:pointer;' /></a>";
					?>
					</div>
...
<div id="boxes">
                ...
                <table border=0 style="padding-bottom: 20px;" cellpadding="5" border="0" cellspacing="0">
					...
					<tr>
					 ...
					 <tr> 
                        ...
                        <a href="agentProfiles.php?agentId='$agentId'" class='btn btn-primary btn-sm'>Cancel</a>
                        </td>
                    </tr>
	...
	<div style="top:150px; left: 551.5px; display: none;" id="dialog" class="window">
	    ...
	    <div align="center" style="margin:5px 0 5px 0;">
            ...
            $amtPaid = $_POST['amount'];
            $paidTimStamp = date('Y-m-d');
            $status = 'Paid';
            include('../conn.php');
            $agentPayment_update="UPDATE tbl_agent_payments SET amtPaid='$amtPaid',status ='$status',paidTimStamp='$paidTimStamp' where profile_id ='$profileId' and agent_id='$agentId'";
            ...
            echo "<a href='agentProfiles.php?agentId=$agentId' class='btn btn-primary btn-sm'>Ok</a>";
            ?>
