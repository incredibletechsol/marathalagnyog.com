		...
		<div class="box col-md-12">
			<div class="box-inner">
				<div class="box-header well" data-original-title="">
					<h2><i class="glyphicon glyphicon-user"></i>Expired Profiles</h2>
				...
				<div class="box-content">
				   ...
				   $fetch_expirydate_dates="select profile_id from tbl_basic_profile where expiry_date>=CURDATE() and expiry_date <= DATE_ADD(CURDATE(),INTERVAL +1 MONTH) ";

  				   $fetch_expiry_date_rs= mysqli_query($conn,$fetch_expirydate_dates);

				    while($fetch_expiry_dates_row = mysqli_fetch_array($fetch_expiry_date_rs)) 
                                        {
                                        $profile_id =$fetch_expiry_dates_row[0]; 
		                          
                        
$fetch_basic_profile="select distinct basicprofile.profile_id,basicprofile.first_name,basicprofile.last_name,							contactinfo.mobile_no,basicprofile.create_datetime,DATE_FORMAT(basicprofile.expiry_date,'%d-%m-%Y'),basicprofile.status from tbl_basic_profile basicprofile,tbl_profile_photo photo,tbl_physical_attr phyattr,tbl_contact_info contactinfo where basicprofile.profile_id=photo.profile_id and basicprofile.profile_id=phyattr.profile_id  and basicprofile.profile_id=contactinfo.profile_id and  basicprofile.profile_id ='$profile_id' order by basicprofile.profile_id desc ";	


					$log->info("Expired Profiles:".$fetch_basic_profile);				
					...
					while($basic_profile_row = mysqli_fetch_array($basic_profile_rs)) 
						{
							...
							if($basic_profile_row [6] == "E") {
							echo "<td><a href='dash.php?id=$basic_profile_row[0]&msg=renew' class='btn btn-success'>Renew</a></td>";
							}
							else	{
							echo "<td><a href='dash.php?id=$basic_profile_row[0]&msg=expired' class='btn btn-danger'>Deactivate</a></td>";
							 }
						        echo "</tr>";
						} 
